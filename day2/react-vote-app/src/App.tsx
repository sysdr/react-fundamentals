...
function App() {
  ...
  const handleVote = async (detail: {
    ...
    setStatus(
      detail.blocked
        ? `Blocked UI for ${detail.durationMs}ms (local votes: ${detail.count})`
        : `Vote recorded (local votes: ${detail.count})`,
    );
    const next: Metrics = {
      ...metrics,
  ...
  return (
    <div className="app-shell">
      <header>
        <h1>Resilient UI: Vote Counter</h1>
        <p className="subtitle">
          Day 2 — useState vote counter, main-thread blocking demo, and live metrics.
        </p>
      </header>
